...
function applyPlanetarySystemSettings() {
  ...
  // Create the sun
  const sunRadius = 15;
  Game.instance.sun = new MassObj(Game.instance.worldCenter, sunRadius, 30000);
  ...
  for (let i = 0; i < asteroids; i++) {
    ...
    obj.velocity = new Vector2(
      -dirrNormalized.y * Math.sqrt(Game.instance.sun.mass / distance) * random,
      dirrNormalized.x * Math.sqrt(Game.instance.sun.mass / distance) * random
    );
